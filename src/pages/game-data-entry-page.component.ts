  ...
  template: `
    <div class="flex flex-col p-4 md:p-6 max-w-7xl mx-auto w-full">
      <div class="flex justify-between items-center mb-4">
        <span class="text-lg md:text-xl font-bold">{{ 'enter-game-data' | transloco }}</span>
      ...
      <div class="mb-4 p-3 rounded-lg bg-blue-50">
        <span class="text-sm font-semibold"> {{ 'game' | transloco }}: {{ event.title }} </span>
      </div>
      ...
      <form action="" [formGroup]="codeInFG" class="flex-col w-full" (submit)="submit()">
        ...
        <mat-vertical-stepper [linear]="false" #stepper>
          <mat-step [hasError]="this.codeInFG.controls.game_id.invalid">
            <ng-template matStepLabel>{{ 'event' | transloco }}: {{ gameSelect.value?.title || ('select-game' | transloco) }}</ng-template>
            <mat-form-field class="w-full">
              <mat-select formControlName="game_id" #gameSelect (valueChange)="changeEvent($event); stepper.next()">
                @for (item of games|async; track item.id) {
...
export class GameDataEntryComponent implements OnInit {
  ...
  player: PlayerDTO[] = [];
  games = this.eventService.getEvents().then((events) => events.filter((e) => e.event_type === 'game'));
  actions?: ActionDTO[];
  ...
  async ngOnInit() {
    ...
    if (this.event && this.event.event_type === 'game') {
      this.codeInFG.controls.game_id.setValue(this.event);
    } else {
      const games = await this.games;
      if (!games || games.length === 0) return;
      this.event = await this.eventService.getEvent(games[0].id);
      if (this.event) {
        this.codeInFG.controls.game_id.setValue(this.event);
      }
    }
  ...
  async changeEvent($event: EventDTO) {
    this.router.navigate([`/${SVB_APP_ROUTES.gameDataEntry}/${$event.id}`]);
    if ($event?.id) {
