...
@Component({
  selector: 'app-training-data-entry',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  template: `
    <div class="flex flex-col p-4 md:p-6 max-w-7xl mx-auto w-full">
      <div class="flex justify-between items-center mb-4">
        <span class="text-lg md:text-xl font-bold">{{ 'enter-training-data' | transloco }}</span>
      ...
      <div class="mb-4 p-3 rounded-lg bg-green-50">
        <span class="text-sm font-semibold"> {{ 'training' | transloco }}: {{ event.title }} </span>
      </div>
      ...
      <form action="" [formGroup]="codeInFG" class="flex-col w-full" (submit)="submit()">
        <mat-vertical-stepper [linear]="false" #stepper>
          <mat-step [hasError]="this.codeInFG.controls.game_id.invalid">
            <ng-template matStepLabel>{{ 'event' | transloco }}: {{ sessionSelect.value?.title || ('select-training' | transloco) }}</ng-template>
            <mat-form-field class="w-full">
              <mat-select formControlName="game_id" #sessionSelect (valueChange)="changeEvent($event); stepper.next()">
                @for (item of trainingSessions|async; track item.id) {
              ...
              <mat-hint>
                <a [routerLink]="['/edit-game', sessionSelect.value]" routerLinkActive="router-link-active">
...
export class TrainingDataEntryComponent implements OnInit {
  ...
  player: PlayerDTO[] = [];
  trainingSessions = this.eventService.getEvents().then((events) => events.filter((e) => e.event_type === 'training'));
  actions?: ActionDTO[];
  ...
  async submit() {
    ...
    const payload = {
      ...
      game_set: null, // Training sessions don't have sets
    };
    ...
    try {
      ...
      if (payload.grade === ActionGrade['#'] && payload.kind === ActionKind.Serve) {
        nextPlayer = this.codeInFG.controls.player_id.value;
      }
  ...
  async ngOnInit() {
    ...
    if (this.event && this.event.event_type === 'training') {
      this.codeInFG.controls.game_id.setValue(this.event);
    } else {
      const sessions = await this.trainingSessions;
      if (!sessions || sessions.length === 0) return;
      this.event = await this.eventService.getEvent(sessions[0].id);
      if (this.event) {
        this.codeInFG.controls.game_id.setValue(this.event);
      }
    }
  ...
  createNewTraining(): void {
    this.router.navigate(['/games/create'], {
      queryParams: { type: 'training' },
    });
  }

  async changeEvent($event: EventDTO) {
    this.router.navigate([`/${SVB_APP_ROUTES.trainingDataEntry}/${$event.id}`]);
    if ($event?.id) {
